...
import { ngoRegister } from "../../services/api";
...
const NGORegister = () => {
  ...
  const validate = () => {
    if (!formData.organizationName || !formData.contactPerson || !formData.email || !formData.password) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await ngoRegister(formData);
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-center text-sm text-secondary-500">
        ...
        <Link to="/ngo/login" className="text-primary-600 hover:text-primary-700 font-medium">
          Sign In
        </Link>
      </p>
    </div>
